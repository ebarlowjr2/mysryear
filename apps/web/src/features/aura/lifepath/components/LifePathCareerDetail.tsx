...
import LifePathFeedbackPanel from './parent/LifePathFeedbackPanel'
...
export default function LifePathCareerDetail({
  career,
  readOnly = false,
  badge = 'A.U.R.A LifePath',
  backHref = '/aura/lifepath/compare',
  ...
  async function loadRelatedResources() {
    ...
    const scholarshipJson = (await scholarshipRes.json().catch(() => null)) as {
      ok?: boolean
      scholarships?: RelatedScholarship[]
    } | null
    const opportunityJson = (await opportunityRes.json().catch(() => null)) as {
      ok?: boolean
    ...
    <div className="space-y-6">
        ...
        <div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <span className="badge">
              ...
              {career.startingSalaryMax.toLocaleString()} start
            </span>
          </div>
        </div>
      ...
      {!simulation ? (
        <div className="card p-6">
          ...
          <p className="mt-2 text-sm text-slate-700">
            {readOnly
              ? 'Read-only career-specific tasks plus any general LifePath tasks for this student profile.'
              : 'Career-specific tasks plus any general LifePath tasks for this student profile.'}
          </p>
          {taskError ? <div className="mt-3 text-sm text-rose-700">{taskError}</div> : null}
          <div className="mt-4 space-y-3">
            {tasks.length === 0 ? (
              <div className="text-sm text-slate-600">
                No saved LifePath tasks yet. Re-save career choices to generate starter tasks.
              </div>
            ) : (
              tasks.map((task) => (
                <button
                  ...
                  disabled={readOnly || simulation}
                  className="w-full rounded-2xl border border-slate-200 p-4 text-left hover:border-blue-300 transition disabled:cursor-default disabled:hover:border-slate-200"
                >
                    ...
                    <div>
                      ...
                      {task.uploaded_files?.file_name ? (
                        <div className="mt-2 text-xs font-semibold text-blue-700">
                          Proof: {task.uploaded_files.file_name}
                        </div>
                      ) : null}
                    </div>
                    <span
                      className={
                        task.status === 'done' ? 'badge bg-emerald-50 text-emerald-700' : 'badge'
                      }
                    >
      ...
      {!simulation ? (
        <LifePathFeedbackPanel
          ...
          careerId={career.id}
          canAdd={canAddFeedback}
        />
      ) : null}

      <div className="card p-6">
        <div className="text-sm font-semibold text-slate-600">
          Related Scholarships + Opportunities
        </div>
        <p className="mt-2 text-sm text-slate-700">
          Matched by career category and related career tags. This will become part of the A.U.R.A
          ...
          <div className="rounded-2xl border border-slate-200 p-4">
            ...
            <div className="mt-3 space-y-3">
              {relatedScholarships.length === 0 ? (
                <div className="text-sm text-slate-600">No related scholarships tagged yet.</div>
              ) : (
                relatedScholarships.map((item) => (
                  ...
                  >
                    ...
                    <div className="mt-1 text-xs text-slate-600">
                      {item.organization || 'Scholarship Provider'}
          ...
          <div className="rounded-2xl border border-slate-200 p-4">
            ...
            <div className="mt-3 space-y-3">
              {relatedOpportunities.length === 0 ? (
                <div className="text-sm text-slate-600">
                  No related business opportunities posted yet.
                </div>
              ) : (
