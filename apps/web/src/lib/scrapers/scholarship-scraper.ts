...
export class ScholarshipScraper {
  ...
  private extractTags(text: string): string[] {
    ...
    if (lowerText.includes('first') && lowerText.includes('generation'))
      tags.push('first-generation')
    ...
    if (lowerText.includes('undergraduate') || lowerText.includes('college'))
      tags.push('undergraduate')
    if (
      lowerText.includes('graduate') ||
      lowerText.includes('masters') ||
  ...
  private extractScholarshipsFromHTML(
    html: string,
  ): Array<{ name: string; amount: string | null; deadline: string | null; link: string | null }> {
    ...
    try {
      ...
      const uniqueScholarships = scholarships.filter(
        (scholarship, index, self) =>
          index ===
          self.findIndex(
            (s) => s.name.toLowerCase().trim() === scholarship.name.toLowerCase().trim(),
          ),
      )
  ...
  async scrapeCapitalOne(): Promise<ScrapedScholarship[]> {
    ...
    try {
      ...
      const html = await this.makeRequest(
        'https://www.capitalone.com/about/corporate-citizenship/education/',
      )
