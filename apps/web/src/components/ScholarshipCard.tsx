...
export default function ScholarshipCard({ s }: { s: Scholarship }) {
  return (
    <a href={s.link} target="_blank" rel="noreferrer" className="card p-5 hover:shadow-lg transition">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="font-bold text-lg">{s.name}</h3>
          <p className="mt-1 text-sm text-slate-600">Deadline: {s.deadline}</p>
          {s.tags && <div className="mt-2 flex flex-wrap gap-2">{s.tags.map(t => <span key={t} className="badge">{t}</span>)}</div>}
        </div>
