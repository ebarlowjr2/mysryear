...
export default async function OpenDashboard() {
  ...
  return (
    <div className="container-prose py-10">
      <h1 className="text-3xl sm:text-4xl font-black tracking-tight">Open Dashboard</h1>
      <p className="mt-2 text-slate-700">Welcome back{session?.user?.name ? `, ${session.user.name}` : ""}.</p>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">{stats.map((s,i)=><StatTile key={i} label={s.label} value={s.value} desc={s.desc}/>)}</div>
      <div className="mt-8 grid md:grid-cols-2 gap-6">
        <div className="card p-6">
          ...
          <div className="mt-4 flex gap-3"><a className="btn-secondary" href="/planner">View Planner</a><a className="btn-primary" href="/open-dashboard/scholarships">Track Scholarships</a></div>
        </div>
        <DocUpload />
      </div>
    </div>
