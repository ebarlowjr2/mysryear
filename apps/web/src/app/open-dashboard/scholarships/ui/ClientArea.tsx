...
export default function ClientArea({ initialDiscover }: { initialDiscover: Row[] }) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') localStorage.setItem('msy_applied', JSON.stringify(applied))
  }, [applied])
  useEffect(() => {
    if (typeof window !== 'undefined') localStorage.setItem('msy_todo', JSON.stringify(todo))
  }, [todo])

  const onSave = (r: Row) => {
    if (!todo.find((x) => x.id === r.id) && !applied.find((x) => x.id === r.id))
      setTodo([r, ...todo])
  }
  const onApply = (r: Row) => {
    if (!applied.find((x) => x.id === r.id)) setApplied([r, ...applied])
    ...
    <div className="mt-6">
      ...
      <div className="grid md:grid-cols-2 gap-6 mt-6">
        <section>
          ...
          <div className="mt-3 space-y-3">
            {filteredTodo.length ? (
              filteredTodo.map((s) => <ScholarshipRow key={s.id} item={s} onApply={onApply} />)
            ) : (
        ...
        <section>
          ...
          <div className="mt-3 space-y-3">
            {filteredApplied.length ? (
              filteredApplied.map((s) => <ScholarshipRow key={s.id} item={s} />)
            ) : (
      ...
      <section className="mt-10">
        ...
        <p className="mt-1 text-slate-600 text-sm">
          From partner feeds and curated sources (mock data).
        </p>
        <div className="mt-3 space-y-3">
