...
export default function StudentDashboardClient() {
  ...
  const scholarshipReadinessValue =
    typeof summary?.scholarships?.readiness?.percentage === 'number'
      ? `${summary.scholarships.readiness.percentage}%`
      : '—'
  const scholarshipValue = new Intl.NumberFormat('en-US', {
    style: 'currency',
    currency: 'USD',
    maximumFractionDigits: 0,
  }).format(summary?.scholarships?.availableValue || 0)

  const viewerRole = (summary?.viewerRole as string | null) || null
  const showParentActionCenter =
    viewerRole === 'parent' || viewerRole === 'guardian' || viewerRole === 'admin'
  ...
  return (
    <div className="container-prose py-14">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-700 mt-2">
            Check in each grading period—track progress year-round.
          </p>
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatTile
          label="Activities"
          value={loadingSummary ? '…' : String(summary?.portfolio?.activitiesCount ?? 0)}
          desc="Clubs, jobs, sports, leadership"
        />
        <StatTile
          label="Service Hours"
          value={loadingSummary ? '…' : String(summary?.portfolio?.serviceHoursTotal ?? 0)}
          desc="Volunteer and community work"
        />
        <StatTile
          label="Achievements"
          value={loadingSummary ? '…' : String(summary?.portfolio?.achievementsCount ?? 0)}
          desc="Awards and honors"
        />
        <StatTile
          label="Certifications"
          value={loadingSummary ? '…' : String(summary?.portfolio?.certificationsCompleted ?? 0)}
          desc={summary?.portfolio?.readinessLabel || 'Portfolio progress'}
        />
        <StatTile
          label="Scholarship Ready"
          value={loadingSummary ? '…' : scholarshipReadinessValue}
          desc={summary?.scholarships?.readiness?.label || 'Profile-based matching'}
        />
      </div>
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="text-lg font-bold">
              {showParentActionCenter ? 'Scholarship Action Center' : 'Scholarship Readiness'}
            </h3>
          ...
          <Link href="/scholarships" className="btn-secondary shrink-0">
            View Scholarships
          </Link>
        </div>
        <div className="mt-5 grid sm:grid-cols-2 lg:grid-cols-5 gap-4">
          <div className="rounded-xl border border-slate-200 p-4">
            ...
            <div className="mt-1 text-2xl font-black">
              {loadingSummary ? '…' : scholarshipReadinessValue}
            </div>
          </div>
          <div className="rounded-xl border border-slate-200 p-4">
            <div className="text-xs font-semibold text-slate-500">Current Matches</div>
            <div className="mt-1 text-2xl font-black">
              {loadingSummary ? '…' : String(summary?.scholarships?.currentMatches ?? 0)}
            </div>
          </div>
          <div className="rounded-xl border border-slate-200 p-4">
            <div className="text-xs font-semibold text-slate-500">Money Available</div>
            <div className="mt-1 text-2xl font-black">
              {loadingSummary ? '…' : scholarshipValue}
            </div>
          </div>
          <div className="rounded-xl border border-slate-200 p-4">
            <div className="text-xs font-semibold text-slate-500">Applications</div>
            <div className="mt-1 text-2xl font-black">
              {loadingSummary ? '…' : String(summary?.scholarships?.applicationsInProgress ?? 0)}
            </div>
            <div className="mt-1 text-xs text-slate-500">
              {summary?.scholarships?.applicationTasks
                ? `${summary.scholarships.applicationTasks.completed}/${summary.scholarships.applicationTasks.total} tasks done`
                : 'No checklist yet'}
            </div>
          </div>
          <div className="rounded-xl border border-slate-200 p-4">
            ...
            <div className="mt-1 text-sm font-bold text-slate-800">
              {summary?.scholarships?.topMissingRequirement || 'Build scholarship matches'}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card p-6 mb-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="text-lg font-bold">
              {showParentActionCenter
                ? 'Opportunities to review with your student'
          ...
          <Link href="/opportunities" className="btn-secondary shrink-0">
            View Board
          </Link>
        </div>
        <div className="mt-5 grid md:grid-cols-3 gap-4">
          {(summary?.opportunities || []).length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-600 md:col-span-3">
              No active opportunities posted yet.
            </div>
          ) : (
            (summary?.opportunities || []).map((opportunity) => (
              ...
              >
                ...
                <div className="mt-2 text-xs text-slate-600">
                  {opportunity.business_profiles?.organization_name ||
                    opportunity.career_category ||
                    'Business partner'}
                </div>
                <div className="mt-1 text-xs text-slate-500">
                  {[opportunity.city, opportunity.state].filter(Boolean).join(', ') ||
                    (opportunity.remote_available ? 'Remote' : 'Location TBD')}
                </div>
              </Link>
            ))
          )}
      ...
      <div className="grid lg:grid-cols-2 gap-8 mb-8">
        ...
        <div className="card p-6">
          <h3 className="text-lg font-bold">
            {showParentActionCenter ? 'Parent Action Center' : 'What to do next'}
          </h3>
          <p className="mt-2 text-sm text-slate-700">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Link href="/opportunities" className="card p-6 hover:shadow-lg transition">
          ...
          <p className="text-slate-600 text-sm">
            Internships, volunteer roles, job shadowing, and workshops
          </p>
        </Link>
