...
function daysUntil(d?: string) {
  if (!d) return null
  const t = new Date(d + 'T00:00:00').getTime() - Date.now()
...
function baseTasks(p: Profile): Task[] {
  const t: Task[] = [
    ...
    mk('Create FSA ID (student + parent)', 'Financial Aid', 'Aug–Sep', '', {
      notes: 'https://studentaid.gov/fsa-id/',
    }),
    mk('Draft academic résumé & activities list', 'Documents', 'Aug–Sep'),
    mk('Identify recommenders + ask in person', 'Applications', 'Aug–Sep'),
    mk('FAFSA opens — start application', 'Financial Aid', 'Oct', undefined, {
      notes: 'https://studentaid.gov/h/apply-for-aid/fafsa',
    }),
...
function icsForTasks(tasks: Task[]) {
  ...
  const lines = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//My SR Year//Planner//EN']
...
export default function PlannerPage() {
  ...
  function clearAll() {
    if (confirm('Reset planner to a fresh baseline?')) {
      setTasks(baseTasks(profile))
      setRecs([])
      setVisits([])
    }
  }
  function exportAllICS() {
    const ics = icsForTasks(tasks)
    download('mysryear_planner.ics', ics, 'text/calendar')
  }
  ...
  return (
    <div className="container-prose py-10">
      ...
      <p className="mt-2 text-slate-700">
        Personalize, track everything in one spot, export to calendar/CSV, and print. Built for
      ...
      <div className="card p-6 mt-6">
        ...
        <div className="grid md:grid-cols-4 gap-4 mt-3">
          ...
          <div>
            ...
            <select
              className="border rounded-2xl px-3 py-2 w-full"
              value={profile.path}
              onChange={(e) => setProfile({ ...profile, path: e.target.value as Path })}
            >
              {[
                'College',
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setProfile({
                  ...profile,
                  testing: e.target.value as 'SAT' | 'ACT' | 'Both' | 'None',
                })
              }
            >
              {['None', 'SAT', 'ACT', 'Both'].map((t) => (
          ...
          <div>
            ...
            <select
              className="border rounded-2xl px-3 py-2 w-full"
              value={String(profile.earlyAction)}
              onChange={(e) => setProfile({ ...profile, earlyAction: e.target.value === 'true' })}
            >
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <button className="btn-primary" onClick={() => regenerateBaseline(false)}>
            Add tasks for my path
          </button>
          <button className="btn" onClick={() => regenerateBaseline(true)}>
            Replace with fresh baseline
          </button>
      ...
      <div className="grid lg:grid-cols-4 sm:grid-cols-2 gap-4 mt-6">
        ...
        <Stat
          label="Next 5 deadlines"
          value={dueSoon.length}
          note={dueSoon.map((t) => `${fmtDate(t.due)} • ${t.title}`).join(' · ') || ''}
        />
      </div>
      <div className="card p-4 mt-4 flex flex-wrap gap-2">
        <button className="btn-secondary" onClick={exportAllICS}>
          Export all deadlines (.ics)
        </button>
        <button className="btn-secondary" onClick={exportTasksCSV}>
          Export tasks (CSV)
        </button>
        <button className="btn-secondary" onClick={exportVisitsCSV}>
          Export visits (CSV)
        </button>
      ...
      <div className="card p-4 mt-6 grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            className={`btn ${view === 'Month' ? 'btn-primary' : 'btn-secondary'} w-full`}
            onClick={() => setView('Month')}
          >
            View by Month
          </button>
          <button
            className={`btn ${view === 'Status' ? 'btn-primary' : 'btn-secondary'} w-full`}
            ...
            {MONTHS.map((m) => (
              <section key={m}>
                ...
                <div className="mt-2 space-y-3">
                  {filtered.filter((t) => t.month === m).length === 0 && (
            ...
            {(['todo', 'doing', 'done'] as const).map((s) => (
              <section key={s}>
                <h3 className="text-lg font-bold capitalize">
                  {s.replace('doing', 'in progress')}
                </h3>
                <div className="mt-2 space-y-3">
                  {filtered.filter((t) => t.status === s).length === 0 && (
                    <div className="text-sm text-slate-500">No tasks in this column.</div>
                  )}
                  {filtered
                    .filter((t) => t.status === s)
                    .map((t) => (
        ...
        <div className="grid sm:grid-cols-3 lg:grid-cols-5 gap-3 mt-3 text-sm">
          {(
            ...
            <label key={k} className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={docs[k as keyof DocKit]}
              ...
              <div className="flex items-start justify-between gap-4 flex-wrap">
                  ...
                  <div className="text-sm text-slate-600 flex flex-wrap gap-3 mt-1">
                    {r.email && (
                      <a
                        ...
                        href={`mailto:${encodeURIComponent(r.email)}?subject=Recommendation%20Request&body=Hi%20${encodeURIComponent(r.name)},%0D%0A%0D%0AThanks%20for%20supporting%20my%20application.%20Here%20are%20deadlines%20and%20details...`}
                      >
                        Email
                      </a>
                    )}
                    {r.requested && (
                      <span>
                        Requested: <b>{fmtDate(r.requested)}</b>
                      </span>
                    )}
                    {r.submitted && (
                      <span>
                        Submitted: <b>{fmtDate(r.submitted)}</b>
                      </span>
                    )}
                  </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    onClick={() => {
                      const submitted =
                        prompt('Enter submitted date (YYYY-MM-DD):', todayISO()) || ''
                  ...
                  <button
                    className="btn"
                    onClick={() => setRecs((prev) => prev.filter((x) => x.id !== r.id))}
                  >
                    Delete
                  </button>
                </div>
            ...
            <div key={v.id} className="card p-4">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="min-w-0">
                  ...
                  <div className="text-sm text-slate-600 flex flex-wrap gap-3 mt-1">
                    <span>
                      Date: <b>{fmtDate(v.date)}</b>
                    </span>
                    {v.rating != null && (
                      <span>
                        Rating: <b>{v.rating}/5</b>
                      </span>
                    )}
                  </div>
                  {v.notes && (
                    <div className="text-sm text-slate-700 mt-1 whitespace-pre-wrap">{v.notes}</div>
                  )}
                </div>
                <div className="flex gap-2">
                  <button
                    className="btn"
                    onClick={() => setVisits((prev) => prev.filter((x) => x.id !== v.id))}
                  >
                    Delete
                  </button>
                </div>
      ...
      <p className="mt-8 text-xs text-slate-500">
        Tip: Share exports with parents/counselors. Double-check dates on official sites.
      </p>
    </div>
  )
}
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="min-w-0">
          ...
          <div className="text-sm text-slate-600 flex flex-wrap gap-3 mt-1">
            ...
            <span>
              Due: <b>{fmtDate(t.due)}</b>
              {t.due ? ` (${daysUntil(t.due)}d)` : ''}
            </span>
  ...
  return (
    <div className="flex gap-2">
      <a
        ...
        href={`mailto:?subject=This%20Week%20Plan&body=${body}`}
        title="Email this week to parent/counselor"
      >
        Email this week
      </a>
      <button
...
function AddVisit({ onAdd }: { onAdd: (v: Omit<Visit, 'id'>) => void }) {
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault()
    if (!name.trim()) return alert('School/Program required')
    onAdd({
      name: name.trim(),
      date: date || undefined,
