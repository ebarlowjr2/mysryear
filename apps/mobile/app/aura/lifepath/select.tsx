...
import {
  getCareerCatalog,
  listLifePathCareerIds,
  listParentSimulationCareerIds,
  saveLifePathCareerIds,
  saveParentSimulationCareerIds,
  type LifePathMode,
...
export default function LifePathSelectScreen() {
  ...
  const filtered = useMemo(
    () =>
      careers.filter((career) => {
        ...
        const matchesQuery =
          !q ||
          [career.title, career.category, career.description, ...career.tags]
            .join(' ')
            .toLowerCase()
            .includes(q)
  ...
  const load = useCallback(async () => {
    ...
    if (isSimulation) setSelected(await listParentSimulationCareerIds(user.id))
    else setSelected(active?.id ? await listLifePathCareerIds(active.id) : [])
    setLoading(false)
  ...
  async function save() {
    if (!user?.id) return
    if (!isSimulation && (!studentProfile?.id || role !== 'student')) {
      Alert.alert(
        'Read-only LifePath',
        'Only the student can change official LifePath career choices.',
      )
      return
    }
    if (isSimulation && !isFamilyRole(role)) {
      Alert.alert(
        'Parent simulation unavailable',
        'Parent Simulation is only available to parent or guardian accounts.',
      )
      return
    }
    ...
    const result = isSimulation
      ? await saveParentSimulationCareerIds(user.id, selected)
      : await saveLifePathCareerIds(studentProfile!.id, user.id, selected)
    setSaving(false)
    if (!result.success) {
    ...
    router.push(
      (isSimulation ? '/aura/lifepath?mode=parent-simulation' : '/aura/lifepath') as never,
    )
  }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <TouchableOpacity
        ...
        onPress={() =>
          goBack(isSimulation ? '/aura/lifepath?mode=parent-simulation' : '/aura/lifepath')
        }
      >
      ...
      <Text style={styles.title}>
        {isSimulation ? 'Choose simulation careers.' : 'Choose your top career interests.'}
      </Text>
      <Text style={styles.subtitle}>
        {selected.length} of 5 selected.{' '}
        {isSimulation
          ? 'These choices stay separate from your student’s official LifePath.'
          : 'Pick at least one to start comparing pathways.'}
      </Text>
          ...
          >
            <View style={styles.cardHeader}>
              ...
              <Ionicons
                ...
                size={26}
                color={isSelected ? ui.primary : ui.textSecondary}
              />
            </View>
      ...
      <TouchableOpacity
        style={[styles.saveButton, !selected.length && styles.disabled]}
        disabled={!selected.length || saving}
        onPress={save}
      >
        <Text style={styles.saveButtonText}>
          {saving ? 'Saving...' : isSimulation ? 'Save Parent Simulation' : 'Save LifePath'}
        </Text>
      </TouchableOpacity>
    </ScrollView>
  )
