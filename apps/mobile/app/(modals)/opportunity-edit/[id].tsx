...
export default function OpportunityEditModal() {
  ...
  const fetchOpportunity = useCallback(async () => {
    ...
    try {
      ...
      if (opp) {
        setOrgName(opp.org_name || '')
        setTitle(opp.title)
        setType(opp.type)
        setDescription(opp.description || '')
        setApplyUrl(opp.apply_url || '')
        setLocationMode(opp.location_mode)
        setState(opp.state || '')
        setCountiesInput(formatCounties(opp.counties))
        setDeadline(opp.deadline ? new Date(opp.deadline) : null)
        setStartDate(opp.start_date ? new Date(opp.start_date) : null)
        setIsPublished(opp.is_published)
      }
    } catch (error) {
