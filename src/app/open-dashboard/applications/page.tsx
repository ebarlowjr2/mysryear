...
const STATUS_OPTIONS: Status[] = ["Not started","In progress","Submitted","Accepted","Waitlisted","Denied"];
const TYPE_OPTIONS: AppType[] = ["College","Trade/Apprenticeship","Military","Scholarship","Other"];
...
export default function ApplicationsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    rows.sort((a,b) => {
      ...
      if (sortKey === "status") return STATUS_OPTIONS.indexOf(a.status) - STATUS_OPTIONS.indexOf(b.status);
  ...
  function updateStatus(id: string, status: Status) {
    setApps(prev => prev.map(a => a.id === id ? { ...a, status, lastUpdated: new Date().toISOString() } : a));
  }

  function updateChecklist(id: string, key: keyof Checklist, val: boolean) {
    setApps(prev => prev.map(a => a.id === id ? {
      ...a, checklist: { ...a.checklist, [key]: val }, lastUpdated: new Date().toISOString()
    } : a));
  }

  function exportCSV() {
    const header = ["Name","Program","Type","Round","Deadline","Status","Portal","Fee","Recommenders","Notes"];
    const rows = apps.map(a => [
      a.name, a.program||"", a.type, a.round, a.deadline||"",
      a.status, a.portal||"", a.fee??"", a.recommenders||"", (a.notes||"").replace(/\n/g," ")
    ]);
  ...
  return (
    <div className="container-prose py-10">
      ...
      <p className="mt-2 text-slate-700">
        Add each school/program, track deadlines & tasks, and mark status. Data saves to your browser (local).
      </p>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
        ...
        <Stat
          ...
          note={counts.nearest ? `${daysUntil(counts.nearest.deadline)} days • ${counts.nearest.name}` : ""}
        />
      </div>
      ...
      <div className="card p-6 mt-8">
        ...
        <AppForm
          ...
          initial={editing || {
            ...
            notes: "",
            status: "Not started",
            checklist: { transcript:false, testScores:false, letters:false, essays:false, appFeePaid:false, fafsa:false },
      ...
      <div className="card p-4 mt-6 grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        />
        <select className="border rounded-2xl px-3 py-2" value={statusFilter} onChange={e=>setStatusFilter(e.target.value as Status | "All")}>
          <option>All</option>
          {STATUS_OPTIONS.map(s => <option key={s}>{s}</option>)}
        </select>
        <select className="border rounded-2xl px-3 py-2" value={sortKey} onChange={e=>setSortKey(e.target.value as "deadline"|"name"|"status")}>
          ...
          <div key={a.id} className="card p-5">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div className="min-w-0">
                <div className="text-lg font-bold truncate">{a.name}{a.program ? ` — ${a.program}` : ""}</div>
                <div className="text-sm text-slate-600 flex flex-wrap gap-3 mt-1">
                  <span><b>{a.type}</b> • {a.round}</span>
                  <span>Deadline: <b>{fmtDate(a.deadline)}</b>{a.deadline ? ` (${daysUntil(a.deadline)}d)` : ""}</span>
                  {a.portal && <a className="text-brand-700 underline" href={a.portal} target="_blank" rel="noreferrer">Portal</a>}
                  {a.fee != null && <span>Fee: ${a.fee}</span>}
                  <span>Status: <b>{a.status}</b></span>
                </div>
              </div>
              <div className="flex gap-2">
                <button className="btn-secondary" onClick={() => setEditingId(a.id)}>Edit</button>
                <button className="btn" onClick={() => updateStatus(a.id, "Submitted")}>Mark Submitted</button>
                <button className="btn" onClick={() => {
                  if (!a.deadline) return alert("Add a deadline first.");
                  download(`${a.name.replace(/\W+/g,"_")}_deadline.ics`, toICS(a), "text/calendar");
                }}>Add .ics</button>
                <button className="btn" onClick={() => removeApp(a.id)}>Delete</button>
              </div>
            </div>
            ...
            {(a.recommenders || a.notes) && (
              <div className="mt-3 text-sm text-slate-700 space-y-1">
                {a.recommenders && <div><b>Recommenders:</b> {a.recommenders}</div>}
                {a.notes && <div className="whitespace-pre-wrap"><b>Notes:</b> {a.notes}</div>}
              </div>
            )}
          </div>
        ))}
    ...
    <form onSubmit={submit} className="grid gap-4">
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          ...
          <input className="border rounded-2xl px-3 py-2 w-full" value={form.name} onChange={e=>set("name", e.target.value)} placeholder="e.g., State University"/>
        </div>
        <div>
          <label className="block text-sm font-semibold">Major / Track (optional)</label>
          <input className="border rounded-2xl px-3 py-2 w-full" value={form.program || ""} onChange={e=>set("program", e.target.value)} placeholder="e.g., Computer Science"/>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-4 gap-4">
        <div>
          ...
          <select className="border rounded-2xl px-3 py-2 w-full" value={form.type} onChange={e=>set("type", e.target.value as AppType)}>
            {TYPE_OPTIONS.map(t => <option key={t} value={t}>{t}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-sm font-semibold">Round</label>
          <select className="border rounded-2xl px-3 py-2 w-full" value={form.round} onChange={e=>set("round", e.target.value as Round)}>
            {ROUND_OPTIONS.map(r => <option key={r} value={r}>{r}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-sm font-semibold">Deadline</label>
          <input type="date" className="border rounded-2xl px-3 py-2 w-full" value={form.deadline || ""} onChange={e=>set("deadline", e.target.value)} />
        </div>
        <div>
          <label className="block text-sm font-semibold">Status</label>
          <select className="border rounded-2xl px-3 py-2 w-full" value={form.status} onChange={e=>set("status", e.target.value as Status)}>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div>
          ...
          <input className="border rounded-2xl px-3 py-2 w-full" value={form.portal || ""} onChange={e=>set("portal", e.target.value)} placeholder="https://..."/>
        </div>
        <div>
          <label className="block text-sm font-semibold">Application Fee (USD)</label>
          <input className="border rounded-2xl px-3 py-2 w-full" value={form.fee ?? ""} onChange={e=>set("fee", e.target.value === "" ? null : Number(e.target.value))} inputMode="numeric" placeholder="e.g., 65"/>
        </div>
        <div>
          <label className="block text-sm font-semibold">Recommenders</label>
          <input className="border rounded-2xl px-3 py-2 w-full" value={form.recommenders || ""} onChange={e=>set("recommenders", e.target.value)} placeholder="Names/emails, comma separated"/>
        </div>
      </div>

      <div>
        <label className="block text-sm font-semibold">Notes</label>
        <textarea className="border rounded-2xl px-3 py-2 w-full min-h-[90px]" value={form.notes || ""} onChange={e=>set("notes", e.target.value)} placeholder="Essay topics, visit dates, aid notes, etc."/>
      </div>
      ...
      <div className="mt-2 flex gap-2">
        <button type="submit" className="btn-primary">{form.id ? "Save changes" : "Add application"}</button>
